...
function handleApply(data){
    ...
    if(appliedCouponCode === "SELL200"){
        ...
        document.getElementById('total-price').innerText = discountedTotalAmount.toFixed(2)
        document.getElementById('products-discount').innerText = discountedAmount.toFixed(2)
    }else{
  ...
  function goToHomeBtn(){
    ...
    productPrice.innerText = "0.00"; // Reset product price to 0.00
    productsDiscount.innerText = "0.00"; // Reset products discount to 0.00
    totalPrice.innerText = "0.00"; // Reset total price to 0.00
  }
